...
{
$data = Alternativemedicine::select()->orderby('id', 'DESC')->paginate(PAGINATION_COUNT);
if (!empty($data)) {
...
return view('alternative_medicine.index', ['data' => $data]);
}
public function create()
{
return view('alternative_medicine.create');
}
...
//check if not exsits
$checkExists = Alternativemedicine::where(['name' => $request->name, 'com_code' => $com_code])->first();
if ($checkExists == null) {
...
Alternativemedicine::create($data);
return redirect()->route('alternatives.index')->with(['success' => 'لقد تم اضافة البيانات بنجاح']);
} else {
...
{
$com_code = auth()->user()->com_code;
$data = Alternativemedicine::select()->find($id);
...
return view('alternative_medicine.edit', ['data' => $data]);     //'total_counter_used'=>$total_counter_used
}
...
$data = Alternativemedicine::select()->find($id);
if (empty($data)) {
return redirect()->route('alternatives.index')->with(['error' => 'عفوا غير قادر علي الوصول الي البيانات المطلوبة !!']);
}
$checkExists = Alternativemedicine::where(['name' => $request->name, 'com_code' => $com_code])->where('id', '!=', $id)->first();
if ($checkExists != null) {
return redirect()->back()
->with(['error' => 'عفوا اسم الوحدة مسجل من قبل'])
->withInput();
...
Alternativemedicine::where(['id' => $id, 'com_code' => $com_code])->update($data_to_update);
return redirect()->route('alternatives.index')->with(['success' => 'لقد تم تحديث البيانات بنجاح']);
} catch (\Exception $ex) {
return redirect()->back()
->with(['error' => 'عفوا حدث خطأ ما' . $ex->getMessage()])
->withInput();
...
{
try {
$item_row = Alternativemedicine::find($id);
if (!empty($item_row)) {
...
// }
$data=Alternativemedicine::where($field1,$operator1, $value1)->orderBy('id','DESC')->paginate(PAGINATION_COUNT);
if (!empty($data)) {
foreach ($data as $info) {
$info->added_by_admin = User::where('id', $info->added_by)->value('name');
if ($info->updated_by > 0 and $info->updated_by != null) {
...
return view('alternative_medicine.ajax_search',['data'=>$data]);
}
}
