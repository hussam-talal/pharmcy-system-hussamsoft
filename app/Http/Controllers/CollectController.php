...
//جدول صلاحية المستخدمين من الخزن
$checkExiststreasuries = get_cols_where_row(new Users_treasuries(), array("treasuries_id", ), array("com_code" => $com_code, "admin_id" => auth()->user()->id, ));
if (!empty($checkExiststreasuries)) {
    //اسم الخزنة الذي مخصص لك عشان التحصيل
$checkExiststreasuries['treasuries_name'] = Treasuries::where('id', $checkExiststreasuries['treasuries_id'])->value('name');
//get Treasuries Balance
$checkExiststreasuries['balance'] = get_sum_where(new Treasuries_transactions(), "money", array("com_code" => $com_code,'treasuries_id'=>$checkExiststreasuries['treasuries_id'] ));
}
$mov_type = get_cols_where(new Mov_type(), array("id", "name"), array("active" => 1, 'in_screen' => 2, 'is_private_internal' => 0), 'id', 'ASC');
$accounts = get_cols_where(new Account(), array("name", "account_number", "account_type"), array("com_code" => $com_code, "active"=>1, "is_main" => 0), 'id', 'DESC');
if (!empty($accounts)) {
foreach ($accounts as $info) {
$info->account_type_name = Account_types::where(["id" => $info->account_type])->value("name");
}
}
$accounts_search = get_cols_where(new Account(), array("name", "account_number", "account_type"), array("com_code" => $com_code, "is_main" => 0), 'id', 'DESC');
...
$admins = get_cols_where(new User(), array("id", "name"), array("com_code" => $com_code), 'id', 'ASC');
return view('collect_transactions.index', ['data' => $data, 'checkExiststreasuries' => $checkExiststreasuries, 
'accounts' => $accounts, 'mov_type' => $mov_type,'treasuries'=>$treasuries,'admins'=>$admins,'accounts_search'=>$accounts_search]);
}
//for collect money
...
if($account_type==2){
    $the_final_Balance=refresh_account_blance_supplier($request->account_number,new Account(),new Supplier(),new Treasuries_transactions(),new purchases(),false);
    }elseif($account_type==3){
    $the_final_Balance=refresh_account_blance_customer($request->account_number,new Account(),new Customer(),new Treasuries_transactions(),new Sales_invoices(),new SalesReturn(),false);
    }
    else{
    $the_final_Balance=refresh_account_blance_General($request->account_number,new Account(),new Treasuries_transactions(),false);
    }
...
$the_final_Balance=refresh_account_blance_supplier($account_number,new Account(),new Supplier(),new Treasuries_transactions(),new purchases() ,true);
return view('collect_transactions.get_account_blance',['the_final_Balance'=>$the_final_Balance]);
...
return view('collect_transactions.ajax_search', ['data' => $data,'totalCollectInSearch'=>$totalCollectInSearch]);
}
}
}