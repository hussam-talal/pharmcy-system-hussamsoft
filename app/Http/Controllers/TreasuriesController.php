...
return view('treasuries.index',['data'=>$data]);    
}
...
$checkExists_isMaster=Treasuries::where(['is_main'=>1,'com_code'=>$com_code])->first();
...
return view('treasuries.edit',['data'=>$data]);
}
...
if($request->is_main==1){
$checkExists_isMaster=Treasuries::where(['is_main'=>1,'com_code'=>$com_code])->where('id','!=',$id)->first();
if($checkExists_isMaster!=null){
return redirect()->back()
->with(['error'=>'عفوا هناك خزنة رئيسية بالفعل مسجلة من قبل لايمكن ان يكون هناك اكثر من خزنة رئيسية'])
->withInput(); 
...
return view('treasuries.ajax_search',['data'=>$data]);
}
}
...
return view("treasuries.details",['data'=>$data,'treasuries_delivery'=>$treasuries_delivery]);
}catch(\Exception $ex){
return redirect()->back()
->with(['error'=>'عفوا حدث خطأ ما'.$ex->getMessage()]);
}
...
return view("treasuries.Add_treasuries_delivery",['data'=>$data,'Treasuries'=>$Treasuries]);
}catch(\Exception $ex){
return redirect()->back()
->with(['error'=>'عفوا حدث خطأ ما'.$ex->getMessage()]);
}
