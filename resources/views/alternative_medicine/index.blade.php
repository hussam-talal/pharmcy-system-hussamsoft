...
@section('content')
      ...
      <div class="card">
        <div class="card-header">
          <h3 class="card-title card_title_center">بيانات   الاصناف البديلة  </h3>
          <input type="hidden" id="token_search" value="{{csrf_token() }}">
          <input type="hidden" id="ajax_search_url" value="{{ route('units.ajax_search') }}">
        
          <a href="{{ route('alternatives.create') }}" class="btn btn-sm btn-success" >اضافة جديد</a>
        </div>
        <!-- /.card-header -->
        <div class="card-body">
          <div class="row">
  ...
  @if($info->updated_by>0 and $info->updated_by!=null )
        ...
        <a href="{{ route('alternatives.edit',$info->id) }}" class="btn btn-sm  btn-primary ml-1">تعديل</a>
        <a href="{{ route('alternatives.delete',$info->id) }}" class="btn btn-sm  btn-danger">حذف</a>   
...
@section('script')
<script src="{{ asset('assets/js/alternatives.js') }}"></script>
