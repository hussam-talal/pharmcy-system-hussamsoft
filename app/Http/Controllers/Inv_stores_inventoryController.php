...
return view('inv_stores_inventory.index', ['data' => $data, 'stores' => $stores]);
}
public function create()
{
$com_code = auth()->user()->com_code;
$stores = get_cols_where(new Store(), array('id', 'name'), array('com_code' => $com_code,'active'=>1), 'id', 'ASC');
return view('inv_stores_inventory.create', ['stores' => $stores]);
}
...
return redirect()->route("stores_inventory.index")->with(['success' => 'لقد تم اضافة البيانات بنجاح']);
} catch (\Exception $ex) {
return redirect()->back()
...
return view('inv_stores_inventory.edit', ['data' => $data,'stores'=>$stores]);
}
...
$info->item_name = items::where('item_code', $info->item_code)->value('name');
$info->item_type = items::where('item_code', $info->item_code)->value('item_type');
...
if($data['is_closed']==0){
$items_in_store=items_batches::where("com_code","=",$com_code)->where("store_id","=",$data['store_id'])->orderby('item_code','ASC')->distinct()->get(['item_code']);
if (!empty($items_in_store)) {
foreach ($items_in_store as $info) {
$info->name = items::where('item_code', $info->item_code)->value('name');
}
}
}else{
$items_in_store="";
}
return view("inv_stores_inventory.show", ['data' => $data, 'details' => $details,'items_in_store'=>$items_in_store]);
} catch (\Exception $ex) {
return redirect()->back()
->with(['error' => 'عفوا حدث خطأ ما' . $ex->getMessage()]);
}
...
$items_in_store=items_batches::where("com_code","=",$com_code)->where("store_id","=",$data['store_id'])->where('item_code','=',$_POST['items_in_store'])->orderby('item_code','ASC')->distinct()->get(['item_code']);
}
if(!empty($items_in_store)){
foreach($items_in_store as $info){
if($_POST['dose_enter_empty_batch']==1){
$info->Batches=items_batches::select("*")->where("com_code",'=',$com_code)->where('item_code','=',$info->item_code)->where('store_id','=',$data['store_id'])->get();
}else{
$info->Batches=items_batches::select("*")->where("com_code",'=',$com_code)->where('item_code','=',$info->item_code)->where('store_id','=',$data['store_id'])->where('quantity','>',0);
}
...
$data_insert['inv_units_id'] = $batch->inv_units_id;
...
return view("inv_stores_inventory.load_edit_item_details", ['parent_pill_data' => $parent_pill_data, 'item_data_detials' => $item_data_detials]);
}
}
}
}
...
$flag=update(new items_batches(),$dataUpdateBatch,array("com_code" => $com_code,'auto_serial'=>$Data_item_details['batch_auto_serial'],'item_code'=>$Data_item_details['item_code']));
if( $flag){
...
$MainUomName = get_field_value(new units(), "name", array("com_code" => $com_code, "id" => $Data_item_details['inv_units_id']));
$items_Data = get_cols_where_row(new items(), array("unit_id", "retail_unit_quntToParent", "retail_unit_id", "does_has_retailunit"), array("com_code" => $com_code, "item_code" => $Data_item_details['item_code']));
//التاثير في حركة كارت الصنف
$dataInsert_items_movements['items_movements_categories'] = 3;
$dataInsert_items_movements['items_movements_types'] = 6;
$dataInsert_items_movements['item_code'] = $Data_item_details['item_code'];
//كود الفاتورة الاب
$dataInsert_items_movements['FK_table'] = $data_parent['auto_serial'];
//كود صف الابن بتفاصيل الفاتورة
$dataInsert_items_movements['FK_table_details'] = $Data_item_details['id'];
$dataInsert_items_movements['byan'] = "جرد بالمخازن للباتش رقم" . " " . $Data_item_details['batch_auto_serial'] . " جرد رقم" . " " . $data_parent['auto_serial'];
//كمية الصنف بكل المخازن قبل الحركة
...
$dataInsert_items_movements["store_id"] = $data_parent['store_id'];
...
if ($flag) {
//update items Quantity mirror  تحديث المرآه الرئيسية للصنف
do_update_itemCardQuantity(
new items(),
$Data_item_details['item_code'],
new items_batches(),
$items_Data['retail_unit_quntToParent']
);
}
}
}
...
return view('inv_stores_inventory.ajax_search', ['data' => $data]);
}
}
...
public function printsaleswina4($id,$size){
    ...
    if($size=="A4"){
        return view('inv_stores_inventory.printsaleswina4',['data'=>$invoice_data,'systemData'=>$systemData,'invoices_details'=>$invoices_details]);
    }else{
        return view('inv_stores_inventory.printsaleswina6',['data'=>$invoice_data,'systemData'=>$systemData,'invoices_details'=>$invoices_details]);
