...
<div class="card">
  <div class="card-header">
    ...
    <input type="hidden" id="ajax_search_url" value="{{ route('purchasesreturn.ajax_search') }}">
  
    {{-- <a href="{{ route('purchasesreturn.store') }}" class="btn btn-sm btn-success" >اضافة جديد</a> --}}
    <form action="{{route('purchasesreturn.store')}}" method="post" class="btn btn-sm btn-success" id="LoadaddDataReturn1" > 
      @csrf
      @can('اضافة مرتجع مشتريات')
      <button type="submit" class="btn btn-primary btn-sm">   اضافة مرتجع </button>   </form>
     @endcan
    </div>
  <!-- /.card-header -->
  <div class="card-body">
  ...
  {{-- <a href="{{ route('purchasesreturn.edit',$info->id) }}" class="btn btn-sm  btn-info">تعديل</a>    --}}
  {{-- <a href="{{ route('purchasesreturn.delete',$info->id) }}" class="btn btn-sm are_you_shue  btn-danger">حذف</a>    --}}

  @endif
  <a style="font-size: .875rem; padding: 0.25rem 0.5rem;color:white" target="_blank" href="{{ route('purchasesreturn.printsaleswina4',[$info->id,'A4']) }}" class="btn btn-primary btn-xs"> طباعة</a>
