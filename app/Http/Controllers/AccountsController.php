...
return view('accounts.index', ['data' => $data, 'account_types' => $account_types]);
}
public function create()
{
$com_code = auth()->user()->com_code;
$account_types = get_cols_where(new Account_types(), array("id", "name"), array("active" => 1, "relatediternalaccounts" => 0), 'id', 'ASC');
$parent_accounts = get_cols_where(new Account(), array("account_number", "name"), array("is_main" => 1, "com_code" => $com_code), 'id', 'ASC');
return view('accounts.create', ['account_types' => $account_types, 'parent_accounts' => $parent_accounts]);
}
...
$data_insert['is_main'] = $request->is_main;
if ($data_insert['is_main'] == 0) {
...
$account_types = get_cols_where(new Account_types(), array("id", "name"), array("active" => 1), 'id', 'ASC');
$parent_accounts = get_cols_where(new Account(), array("account_number", "name"), array("is_main" => 1, "com_code" => $com_code), 'id', 'ASC');
return view('accounts.edit', ['account_types' => $account_types, 'parent_accounts' => $parent_accounts, 'data' => $data]);
}
...
return view('accounts.ajax_search', ['data' => $data]);
}
}
}