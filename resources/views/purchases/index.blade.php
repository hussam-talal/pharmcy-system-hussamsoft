...
@section('content')
      ...
      <div class="card">
        <div class="card-header">
          ...
          <input type="hidden" id="ajax_search_url" value="{{ route('purchases.ajax_search') }}">
          @can('أضافة مشريات')
          <a href="{{ route('purchases.create') }}" class="btn btn-sm btn-success" >اضافة جديد</a>
        @endcan
        </div>
        <!-- /.card-header -->
        <div class="card-body">
        ...
        <div id="ajax_responce_serarchDiv">
         ...
         @foreach ($data as $info )
            <tr >
              ...
              <td style="width: 17%" >{{ $info->supplier_name }}</td>  
             ...
             <td>@if($info->is_approved==1)  مرحلة   @else   مفتوحة @endif</td> 

         <td>

          @if($info->is_approved==0)
          @can('تعديل مشريات')
        <a href="{{ route('purchases.edit',$info->id) }}" class="btn btn-sm  btn-primary">
         <i class="nav-icon fas fa-edit"></i> تعديل
        </a>  
      @endcan
        @can('حذف مشريات')
        <a href="{{ route('purchases.delete',$info->id) }}" class="btn btn-sm are_you_shue btn-danger">
        ...
        @can(' تفاصيل مشريات')
        <a href="{{ route('purchases.show',$info->id) }}" class="btn btn-sm btn-info">
          <i class="nav-icon fas fa-details"></i>التفاصيل
        </a>   
        @endcan 
        <a style="font-size: .875rem; padding: 0.25rem 0.5rem;color:white" target="_blank" href="{{ route('purchases.printsaleswina4',[$info->id,'A4']) }}" class="btn btn-primary btn-xs">
