$(document).ready(function () {
  ...
  function get_item_unit_price() {
    ...
    jQuery.ajax({
      ...
      dataType: 'json',
      cache: false,
      data: { item_code: item_code, unit_id: unit_id, sales_item_type: sales_item_type, "_token": token },
  ...
  function get_item_units() {
    ...
    if (item_code != "") {
      ...
      var ajax_get_item_units_url = $("#ajax_get_item_units").val();
      jQuery.ajax({
  ...
  function get_item_batches(oldBatchId = null) {
    var item_code = $("#item_code").val();
    var unit_id = $("#unit_id").val();
    ...
    if (item_code != "" && unit_id != "" && store_id != "") {
      ...
      jQuery.ajax({
        ...
        dataType: 'html',
        cache: false,
        data: { item_code: item_code, unit_id: unit_id, store_id: store_id, "_token": token_search },
        success: function (data) {
  ...
  //     dataType: 'html',
  //     cache: false,
  //     data: { invoiceautoserial: invoiceautoserial, "_token": token_search },
  //     success: function (data) {
  //       $("#refresh_main_pill").html(data);
  ...
  // }
  $(document).on('click', '#approveToIvoiceDetailsActive', function (e) {
    ...
    jQuery.ajax({
      ...
      success: function (data) {
        ...
        $("#approve_invoice_Modal_body").html(data);
        $("#approve_invoice_Modal").modal("show");
        recalcualte();
      },
      error: function () {
        alert("erro");
  ...
  function recalcualte1() {
    ...
    if (discount_type != "") {
      if (discount_type == 1) {
        ...
        if (discount_percent == "") { discount_percent = 11; }
        discount_percent = parseFloat(discount_percent);
        var discount_value = total_cost_items * discount_percent / 100;
        $("#discount_value").val(discount_value * 1);
        var total_cost = total_cost_items - discount_value;
        ...
        var total_cost = total_cost_items - discount_percent;
        $("#total_cost").val(total_cost * 1);
      }
      ...
      var total_cost = total_cost_items;
  ...
  function recalculate_itemTotlaRow() {
    ...
    $("#total_price").val((parseFloat(item_quantity) * parseFloat(item_price)) * 1);
  ...
  $(document).on('click', '#AddItemToIvoiceDetailsRow', function (e) {
    ...
    var item_total = $("#total_price").val();
    if (item_total == "") {
    ...
    jQuery.ajax({
      ...
      cache: false,
      data: {
        "_token": token_search, store_id: store_id, sales_item_type: sales_item_type, 
        item_code: item_code,unit_id: unit_id, inv_itemcard_batches_autoserial:inv_itemcard_batches_autoserial,
        item_quantity: item_quantity, item_price: item_price,item_total: item_total, store_name: store_name,
        unit_id_name: unit_id_name,sales_item_type_name: sales_item_type_name, ismainunit: ismainunit, item_code_name: item_code_name
      },
      success: function (data) {
...
function make_enter_add(){
  ...
  var sales_item_type = $("#sales_item_type_main").val();
  // if (sales_item_type == "") {
  //   alert("من فضلك اختر نوع البيع ");
  //   $("#sales_item_type").focus();
  //   return false;
  ...
  jQuery.ajax({
    ...
    data: {
      ...
      unit_id: unit_id, inv_itemcard_batches_autoserial: inv_itemcard_batches_autoserial,
  ...
  function recalcualte() {
    ...
    // if($("#AddItemToIvoiceDetailsActive").length){
    //   return false;
    // }
...
// var sales_item_type_mainUpdate=$("#sales_item_type_mainUpdate").val();
    ...
    jQuery.ajax({
      ...
      data: {
        ...
        total_befor_discount: total_befor_discount,discount_type: discount_type,
         discount_percent: discount_percent,discount_value: discount_value, total_cost: total_cost,
        notes: notes, invoice_date: invoice_date,pill_type: pill_type,customer_code: customer_code, 
  ...
  $(document).on('click', '#DoApproveInvoiceFinally', function (e) {
   ...
   var total_cost_items=$("#total_cost_items").val;
    ...
    if (discount_type == 1) {
      ...
      var discount_value = $("#discount_value").val();
      if (discount_value > total_cost_items) {
  ...
  $(document).on('mouseenter', '#1DoApproveInvoiceFinally11', function (e) {
  ...
  function make_search() {
    ...
    jQuery.ajax({
      ...
      cache: false,
      data: {
        "_token": token, customer_code: customer_code,  Sales_matrial_types: Sales_matrial_types, pill_type: pill_type, 
        is_approved: is_approved, invoice_date_from: invoice_date_from, invoice_date_to: invoice_date_to, 
        searchbyradio:searchbyradio,search_by_text:search_by_text
